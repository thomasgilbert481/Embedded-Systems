    ...
    if (sw1_debounce > 0) {             // Are we in the debounce cooldown period?
        ...
        if (!(P4IN & SW1)) {            // Is SW1 currently pressed? (active low)
            ...
            if (event == NONE) {                // Is the car idle?
                ...
                //--------------------------------------------------------------
                switch (next_shape) {
                    case CIRCLE:    next_shape = CIRCLE_L;  break;  // Right circle -> Left circle
                    case CIRCLE_L:  next_shape = FIGURE8;   break;  // Left circle -> Figure-8
                    case FIGURE8:   next_shape = TRIANGLE;  break;  // Figure-8 -> Triangle
                    case TRIANGLE:  next_shape = CIRCLE;    break;  // Triangle -> back to start
                    default:        next_shape = CIRCLE;    break;
                }
            }
