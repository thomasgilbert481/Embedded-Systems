//==============================================================================
// File Name: macros.h
// Description: All #define constants for Project 8 — Serial Communication
...
//------------------------------------------------------------------------------
// Serial / UART Constants (Carlson Lecture 13 / Project 8 guide)
//------------------------------------------------------------------------------
#define BEGINNING           (0)      // Generic zero / start-of-buffer value

// Ring buffer size — power of 2 recommended for fast modulo wrapping
#define IOT_RING_SIZE       (32)
...
//------------------------------------------------------------------------------
// Baud rate indices (used by baud_rate_index variable and switch statements)
//------------------------------------------------------------------------------
#define BAUD_9600           (0)
#define BAUD_115200         (1)
#define NUM_BAUD_RATES      (2)      // SW2 wraps back to BAUD_9600 after this many
...
//
// MCTLW = (UCSx << 8) | (UCFx << 4) | UCOS16
//   9600:   (0x49 << 8) | (1 << 4) | 1 = 0x4900 | 0x10 | 0x01 = 0x4911
//  115200:  (0x55 << 8) | (5 << 4) | 1 = 0x5500 | 0x50 | 0x01 = 0x5551
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
#define SERIAL_CR           (0x0D)   // Carriage return — appended to TX; stripped on RX
#define SERIAL_LF           (0x0A)   // Line feed     — appended to TX; stripped on RX
#define SERIAL_NULL         (0x00)   // Null terminator for TX buffer sentinel
