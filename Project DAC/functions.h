...
// DAC functions (Project DAC)
void Init_REF(void);
void Init_DAC(void);
void Run_DAC_Demo(void);

// DAC global variables (defined in dac.c and main.c)
extern volatile unsigned int DAC_data;   // Current 12-bit DAC code (0-4095)
extern volatile unsigned int dac_state;  // DAC demo state machine state
extern volatile unsigned int dac_timer;  // Ticks within current DAC state
extern volatile unsigned int dac_running; // TRUE = CCR0 ISR increments dac_timer
