...
void run_circle_left(void) {
    unsigned int total_travel = CIRCLE_L_TRAVEL * 2;  // 2 circles

    if (time_change) {
        time_change = 0;
        if (segment_count <= total_travel) {
            if (right_motor_count++ >= CIRCLE_L_RIGHT_COUNT) {
                P6OUT &= ~R_FORWARD;
            }
            if (left_motor_count++ >= CIRCLE_L_LEFT_COUNT) {
                P6OUT &= ~L_FORWARD;
            }
...
void Run_Circle_Left(void) {
    switch (state) {
        ...
        case START: start_case();       break;
        case RUN:   run_circle_left();  break;
...
void run_figure8(void) {
    ...
    if (time_change) {                              // Only act on new timer ticks
        ...
        if (shape_phase < total_phases) {           // Still have phases left to do?
            ...
            if ((shape_phase & 1) == 0) {           // Even phases (0, 2): curve in direction A
...
void run_triangle(void) {
    ...
    if (time_change) {                              // Only act on new timer ticks
        ...
        if (shape_phase < total_phases) {           // Still have phases left?
            ...
            if ((shape_phase & 1) == 0) {             // Even phases: STRAIGHT
    ...
    if (event == NONE) {                                // Only start if car is idle
        ...
        switch (shape) {
            case CIRCLE:                                // Circle selected
                strcpy(display_line[0], "  Circle R ");  // Write "Circle" to LCD line 1
                break;
            case CIRCLE_L:
                strcpy(display_line[0], " Circle L ");
                break;
