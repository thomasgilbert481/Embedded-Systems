...
void Init_Port2(void){
    ...
    // P2.4 -- IOT_RUN LED: Output, start OFF
    P2SEL0 &= ~IOT_RUN_RED;
    P2SEL1 &= ~IOT_RUN_RED;
    P2OUT  &= ~IOT_RUN_RED;
    P2DIR  |=  IOT_RUN_RED;
...
void Init_Port3(void){
    ...
    // P3.6 -- IOT_LINK LED: Output, start OFF
    P3SEL0 &= ~IOT_LINK_CPU;
    P3SEL1 &= ~IOT_LINK_CPU;
    P3OUT  &= ~IOT_LINK_CPU;
    P3DIR  |=  IOT_LINK_CPU;
    IOT_LINK_DIR |=  IOT_LINK_PIN;
    IOT_LINK_PORT &= ~IOT_LINK_PIN;

    // P3.7 -- IOT_EN: Output, start LOW (holds ESP32 in reset)
    P3SEL0 &= ~IOT_EN_CPU;
    P3SEL1 &= ~IOT_EN_CPU;
    P3OUT  &= ~IOT_EN_CPU;
    P3DIR  |=  IOT_EN_CPU;
    IOT_EN_DIR  |=  IOT_EN_PIN;
    IOT_EN_PORT &= ~IOT_EN_PIN;     // LOW = reset held
}
...
void Init_Port4(void){
    ...
    // P4.2 -- UCA1RXD: eUSCI_A1 UART receive (PC backchannel)
    // UART function: SEL0 = 1, SEL1 = 0
    P4SEL0 |=  UCA1RXD;
    P4SEL1 &= ~UCA1RXD;

    // P4.3 -- UCA1TXD: eUSCI_A1 UART transmit (PC backchannel)
    // UART function: SEL0 = 1, SEL1 = 0
    P4SEL0 |=  UCA1TXD;
    P4SEL1 &= ~UCA1TXD;
...
void Init_Port5(void){
    ...
    P5OUT  |=  IOT_BOOT_CPU;
    P5DIR  |=  IOT_BOOT_CPU;
    IOT_BOOT_DIR  |=  IOT_BOOT_PIN;
    IOT_BOOT_PORT |=  IOT_BOOT_PIN; // HIGH = normal boot mode
}
...
void Init_Port6(void){
    ...
    // P6.1-P6.4 -- Motor pins: Timer B3 PWM function (SEL0=1, SEL1=0)
    // TB3CCR1=R_FORWARD, TB3CCR2=L_FORWARD, TB3CCR3=R_REVERSE, TB3CCR4=L_REVERSE
    P6SEL0 |=  R_FORWARD;
    P6SEL1 &= ~R_FORWARD;
    P6DIR  |=  R_FORWARD;
